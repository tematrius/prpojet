...
if ($doc['est_restreint']) {
    // Vérifie l'autorisation active
    $stmt = $pdo->prepare("SELECT expiration_acces FROM demandes WHERE id_document = ? AND id_demandeur = ? AND statut = 'accepte' ORDER BY expiration_acces DESC LIMIT 1");
